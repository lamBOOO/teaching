...
# ╔═╡ 7629e50f-23aa-42fc-875a-8f0e5d0518d1
html"<button onclick='present()'>present</button>"

# ╔═╡ 6b97895b-9f34-4478-8564-e93efaaa2f82
md"""
...
We consider an grayscale image `` A \in \mathbb{R}^{n \times n} `` with entries $A_{ij} \in [0,1]$ representing the gray intensity. A square matrix $A \in \mathbb{R}^{n \times n}$ can be written as SVD, defined as:
```math
...
We construct the compressed image `` \tilde{A} \in \mathbb{R}^{n \times n} `` as rank ``r`` approximation, defined as:
```math
...
@bind r Slider(1:size(img)[1], default=Int(round(size(img)[1]//20)), show_value=true)
